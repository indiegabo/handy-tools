...
{
    public static class HandyAESEncryption
    {
        public static string newIV => GenerateIV();
        ...
        /// <param name="plainText">The already existent iv</param>
        ...
        public static string EncryptFromExistentIV(string plainText, string iv, string password)
        {
            using (var aes = Aes.Create())
            {
        ...
        {
            return Decrypt(encryptedText.text, encryptedText.iv, password);
        }
        ...
        private static byte[] ConvertToKeyBytes(SymmetricAlgorithm algorithm, string password)
        {
            ...
            if (keyBytes.Length != validKeySize)
            {
                ...
                Array.Copy(
                    keyBytes,
                    newKeyBytes,
                    System.Math.Min(keyBytes.Length, newKeyBytes.Length)
                );
