using System;
using System.Collections.Generic;
using IndieGabo.HandyTools.Modules;

namespace IndieGabo.HandyTools.FSMModule
{
    /// <summary>
    /// Centralizes metadata and dependency rules for the FSM module.
    /// </summary>
    public static class FSMModuleDefinition
    {
        private static readonly IReadOnlyList<HandyModuleDependencyStatus> _dependencies =
            Array.Empty<HandyModuleDependencyStatus>();

        /// <summary>
        /// Gets the stable runtime descriptor for the FSM module.
        /// </summary>
        public static HandyModuleDescriptor Descriptor { get; } = new(
            "fsm",
            "FSM",
            "Provides finite state machine brains, states, transitions, triggers, and editor visualization tooling.",
            HandyModuleActivationMode.Optional,
            180,
            isActiveByDefault: true
        );

        /// <summary>
        /// Gets the dependency status list for the FSM module.
        /// </summary>
        public static IReadOnlyList<HandyModuleDependencyStatus> Dependencies =>
            _dependencies;

        /// <summary>
        /// Gets whether the FSM module is currently active.
        /// </summary>
        public static bool IsActive => HandyModuleSettings.Instance.IsModuleActive(Descriptor);
    }
}